...
async function fetchCards() {
  for (const player of DRAFT_PLAYERS) {
    const url = `http://localhost:3005/api/draft/og?id=${player.id}&v=${player.stats.vision}&e=${player.stats.execution}&c=${player.stats.chaos}&d=${player.stats.defense}&f=${player.stats.flair}`;
