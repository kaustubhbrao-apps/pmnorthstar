...
`;
  ...
  // Scalars only, in their own module. The CheckIt page is a client
  // component; importing YC_STUDY there would ship all 496 rows in the
  // browser bundle, since an object's properties don't tree-shake.
  const summary = {
    ranAt: study.ranAt,
    audited: study.audited,
    median: study.median,
    topName: rows[0].name,
    topScore: rows[0].score,
  };
  const summaryOut = `// AUTO-GENERATED by scripts/yc-audit-study.ts alongside yc-study.ts.
// Client-safe subset: scalars only, no per-company rows.

export const YC_STUDY_SUMMARY = ${JSON.stringify(summary, null, 2)} as const;
`;
  fs.writeFileSync(path.join(OUT_DIR, "yc-study-summary.ts"), summaryOut, "utf8");
